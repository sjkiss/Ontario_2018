...
on18 %>% 
  select(starts_with('imm_'), starts_with('econ_')) %>% ggpairs(., lower=list(continuous='smooth'),labeller = label_wrap_gen(10))

#Econ correlation
cor(on18$econ_sentiment, on18$immigration_sentiment, use='complete.obs')
on18 %>% 
  filter(econ_sentiment > -4&econ_sentiment < 4) %>% 
  ggplot(., aes(x=econ_sentiment, y=immigration_sentiment))+geom_point()+geom_smooth(method='lm')
...
immigration %>% 
  ...
  spread(sentiment, proportion, fill=0)-> immigration_proportion
...
financial %>% 
  ...
  group_by(id, sentiment) %>% 
  mutate(proportion=n/sum) %>% 
  spread(sentiment, proportion, fill=0)-> financial_proportion 


immigration_proportion %>% 
  select(id, prop_immigration_negative=negative, prop_immigration_positive=positive) %>% 
  full_join(on18, .)-> on18

financial_proportion %>% 
  select(id, prop_financial_negative=negative, prop_financial_positive=positive) %>% 
  full_join(on18, .)-> on18
names(on18)
on18 %>% 
  select(starts_with('prop')) %>% 
  cor(., use='complete.obs')-> cors
cors
cors %>% 
  as.data.frame(cors) %>% 
